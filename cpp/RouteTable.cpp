...
const string RouteTable::DBName = "RouteTable";
...
void RouteTable::store2DB()
{
    ...
    if (!documents.empty())
    {
        ...
        auto collection = (*client)[DBName][to_string(shardIndex)];
    ...
    */
    ...
    vector<mongocxx::model::update_one> updates;

    for (const auto &item : adjRIB)
    {
        bsoncxx::builder::basic::document filter{};
        filter.append(bsoncxx::builder::basic::kvp("AS", (int32_t)this->ASN));
        filter.append(bsoncxx::builder::basic::kvp("prefix", Trie::getValue(item.first)));
    ...
    if (!updates.empty())
    {
        ...
        {
            bsoncxx::builder::basic::document indexBuilder;
            indexBuilder.append(bsoncxx::builder::basic::kvp("AS", 1)); // 升序索引
            indexBuilder.append(bsoncxx::builder::basic::kvp("prefix", 1));
...
void RouteTable::fetchFromDB(const vector<string> &prefixes)
{
    ...
    {
        auto client = MongoManager::acquire();
        auto collection = (*client)[DBName][to_string(this->ASN % 2000)];
